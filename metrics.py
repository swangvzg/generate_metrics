...
def detected_trs_metrics(csv_file, portion=0.1, fov_size=0.202):
    ...
    for line in smart_open.open(csv_file):
        ...
        if line_count % 1000000 == 0:
            if line_count < 1e7:
                print(datetime.datetime.now(), f" - Processed {line_count} lines ...")
            else:
        ...
        if(line_count == 1): # first line
            ...
            if random.randint(0,1000) < 1000*portion:
    ...
    return qc_metrics, trs, trid_count, barcode_count, data_picked_df, z_error_rates, z_counts, global_x_min, global_x_max, global_y_min, global_y_max
